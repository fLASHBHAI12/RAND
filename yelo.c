...
void usage() {
    printf("Usage: ./TF ip port time threads\n");
    exit(1);
}
...
// Function to check if the current date is past the expiry date (04/02/2025)
int is_expired() {
    struct tm expiry_date = {0};
    struct tm current_date = {0};
    
    // Set the expiry date (04/02/2025)
    expiry_date.tm_year = 2025 - 1900;  // Years since 1900
    expiry_date.tm_mon = 1;             // February (0-indexed)
    expiry_date.tm_mday = 4;            // 4th day
    ...
    // Compare dates
    if (difftime(mktime(&current_date), mktime(&expiry_date)) > 0) {
        return 1;  // Expired
    }
...
int main(int argc, char *argv[]) {
    ...
    if (is_expired()) {
        printf("EXPIRED BY TF FLASH92 ON 04/02/2025\n");
        return 1;
    }
